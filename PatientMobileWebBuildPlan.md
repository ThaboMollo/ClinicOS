...
- **UI**: Tailwind CSS
- **Design system**: ClinicOSTheme.md (Inter font, Blue #0B5AA8, Teal #20C997)
- **Icons**: `lucide-react`
...
- [ ] Install additional dependencies:
  ...
  - `lucide-react`
- [ ] Configure Tailwind with ClinicOS tokens:
  - Primary: `#0B5AA8`
  - Primary Dark: `#083E78`
  - Accent: `#20C997`
  - Accent Dark: `#0FAE7B`
...
- [ ] Create shared UI primitives:
  ...
  - `Spinner`
- [ ] Add ClinicOS logo/icon (from `ClinicOS_icon.png`)

**Acceptance**
- Routes render on mobile screen sizes
- ClinicOS color system applied via Tailwind config
- UI primitives consistent with ClinicOSTheme.md
...
- [ ] Show ClinicOS icon (small, top of screen)
...
- [ ] On submit → call `join-queue` Edge Function
  ...
  - On success:
    - Store `{ appointmentId, accessToken }` in localStorage key `clinicos_session`
...
- [ ] Render questions dynamically by `question_type`:
  - `text` → `<Input>`
  - `dropdown` → `<select>` styled to match ClinicOS inputs
...
Phase 0: Supabase schema + Edge Functions
    ↓
Phase A: App skeleton + ClinicOS theme + routing
    ↓
Phase B: Clinic landing + join queue
    ↓
