# ClinicOS — Value Roadmap & Implementation Spec (v1 → v2)
...
- Two messages max per visit (cost + anti-spam):
  1. On join: "You're #7 at ClinicOS Demo Clinic. Track your place: {link}" — this also solves session recovery if the browser tab is lost.
...
**Implementation.** Client-only. On successful join, store `{ name, phone }` in a separate localStorage key (`clinicos_patient`, distinct from the session). On the join form, prefill both fields with a "Not you? Clear" link. No schema or API changes; the server already reconciles by normalized phone. Explicitly *not* auto-submit — the visible confirm step is the privacy guard on shared phones.
...
3. **Production deployment** — the demo runs off localhost. Vercel project + custom domain (`app.clinicos.co.za` or similar), env vars in Vercel, QR posters must point at the real domain.
